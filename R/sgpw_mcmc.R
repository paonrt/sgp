#' @title Spatial Gaussian Processes Modeling Weibull Distributions

#' @description
#' Markov chain Monte Carlo (MCMC) algorithm for semi-parametric spatial model.

#' @usage
#' sgpw_mcmc = function(nsim, W, S, Sstar, startingPoint = NULL,
#'                       details = FALSE, forecasting = TRUE, burn = 0,
#'                       thin = 1, verbose = +Inf)

#' @details
#' MCMC sampler for semi-parametric spatial model without the binomial random
#'  variables. Details are the same of \code{sgpbw_mcmc} except the part for
...
sgpw_mcmc = function(
  ...
  if (!is.infinite(verbose)) {
    print(paste("sgpw_mcmc: start time at ", Sys.time(), sep = ""))
  }
  ...
  ### storing arrays
  keep_mix_psi = matrix(nrow = 2, ncol = nsim)
  rownames(keep_mix_psi) = c("gamma", "delta")

  keep_mix_sigmaSq = matrix(nrow = 2, ncol = nsim)
  rownames(keep_mix_sigmaSq) = c("gamma", "delta")

  keep_mix_tauSq = matrix(nrow = 2, ncol = nsim)
  rownames(keep_mix_tauSq) = c("gamma", "delta")
  ...
  rownames(keep_sigmaSq) = c("gamma", "delta")

  keep_tauSq = matrix(nrow = 2, ncol = nsim)
  rownames(keep_tauSq) = c("gamma", "delta")

  keep_psi = matrix(nrow = 2, ncol = nsim)
  rownames(keep_psi) = c("gamma", "delta")
  ...
  for (insim in (1 - burn):nsim) {
    ...
    keep_mix_tauSq[, insim] = c(
      mix_tauSq_gamma, mix_tauSq_delta#, mix_tauSq_pi
    )
  ...
  if (!is.infinite(verbose)) {
    print(paste("sgpw_mcmc: end time at ", Sys.time(), sep = ""))
  }
